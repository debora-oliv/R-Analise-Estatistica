...
tabela_freq_fumo_ordenada <- sort(table(survey$Smoke), decreasing = TRUE) #ordenada
...
#Criando uma nova coluna (AgeGroup) no dataframe survey convertendo as idades em grupos
survey$AgeGroup <- cut(survey$Age,
                       breaks = c(10, 20, 30, 40, 80),
